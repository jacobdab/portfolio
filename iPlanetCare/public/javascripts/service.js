...
let showResults = function(arg) {
    ...
    ;
    let jqxhr = $.get('/service/search?q=' + value, (data) => {
        console.log(data);
        $('#searchResults').html('');
    })
        .done((data) => {
            if (data.length === 0) {
                $('#searchResults').append("<p class='lead text-center mt-2'>Brak wyników</p>");
            } else {
                data.forEach(dataE => {
                    if(dataE.model){
...
let showClient = function(arg) {
    ...
    let jqxhr = $.get('/service/search?q=' + value, (data) => {
        ...
        .done((data) => {
            if (data.length === 0) {
                $('#searchClient').append("<p class='lead text-center mt-2 text-white'>Brak wyników</p>");
            } else {
                data.forEach(dataE => {
                    if(dataE.model){
                        $('#searchClient').append("<a href='#'><p class='m-2 lead text-white'>" + dataE.client[0].clientName+"</p></a>")
                        $('#searchClient a').attr('href',"/service/devices/"+dataE.client[0]._id+"/addDevice")
                    }else {
                        $('#searchClient').append("<a href='#'><p class='m-2 lead text-white'>" + dataE.clientName + "</p></a>")
                        $('#searchClient a').attr('href', "/service/devices/" + dataE._id + "/addDevice")
                    }});
            }
        })
        .fail((err) => {
...
$(document).on('click','#napr',function (e) {
    ...
    $.post("/service/devices/"+ deviceId +"/changeStatus?_method=PUT",
...
$('.modalSave').click((e)=>{
    ...
    $.ajax({
        url: '/service/devices/'+loc+'/addRepair',
        method: 'post',
        dataType: 'json',
...
if(location.href === 'http://localhost:4000/service/' || location.href === 'http://localhost:4000/service' ){
    console.log('das');
    fetch('http://localhost:4000/service/data').then((res)=>{
    if(!res.ok){
